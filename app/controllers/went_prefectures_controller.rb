class WentPrefecturesController < ApplicationController
	def create
		@place = Place.find(params[:id])
		went = Went.where(user_id: current_user.id).last
		went_place = @place.went_prefectures.where(went_id: went.ids)
		went_prefecture = WentPrefecture.where(went_id: went.ids, prefecture_id: @place.prefecture_id)
		binding.pry
		if went_prefecture.empty?
			went.update(prefecture_count: went.prefecture_count + 1)
			went_prefecture = WentPrefecture.new
			went_prefecture.went_id = went.ids
